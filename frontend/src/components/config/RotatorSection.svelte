...
<section class="bg-surface-section border border-stroke-section rounded-lg px-4 py-3">
  <div class="flex items-center justify-between {rotatorEnabled ? 'mb-3' : ''}">
    ...
    <label class="text-fg-label text-xs">
      <input
        ...
        on:change={(e) => dispatch("fieldchange", { key: "rotator_enabled", value: e.target.checked })}
      />
      Enable
    </label>
  </div>
  ...
  {#if rotatorEnabled}
    <div class="flex flex-col gap-1.5">
      ...
      <div class="flex items-center gap-2">
        <label class="w-field-xs flex-shrink-0 text-fg-label text-2xs">Threshold</label>
        <input
          type="number"
          class="flex-none w-field-xs"
      ...
      <div class="flex items-center gap-2">
        <label class="w-field-xs flex-shrink-0 text-fg-label text-2xs">Park</label>
        <input
          type="number"
          class="flex-none w-field-xs"
