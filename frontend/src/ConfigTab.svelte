<script>
  ...
  async function doSaveAdvanced() {
    ...
    if (err) { advStatus = 'Error: ' + err; }
    else { advStatus = 'Saved ✓'; setTimeout(() => { advStatus = ''; showAdvancedModal = false; }, 1500); }
  }
  ...
  $: radioType = cfg ? (
    cfg.profiles[cfg.profile]?.flrig_ena ? 'flrig' :
    cfg.profiles[cfg.profile]?.hamlib_ena ? 'hamlib' : 'none'
  ) : 'none';
...
<div class="config-tab">
  ...
  <section>
    ...
    <div class="row">
      ...
      <input id="wl-url" type="text" class="field-input"
    ...
    <div class="row">
      ...
      <input id="wl-key" type="text" class="field-input"
    ...
    <div class="row">
      ...
      <select id="wl-station" class="field-input"
        on:change={e => setProfileField('wavelog_id', e.target.value)}>
        <option value="0" selected={activeProfile().wavelog_id === '0' || activeProfile().wavelog_id === 0}>— select —</option>
        {#each stations as s}
          <option value={s.station_id}
            selected={String(s.station_id) === String(activeProfile().wavelog_id)}>
            {s.station_callsign} ({s.station_profile_name})
          </option>
        {/each}
      </select>
      <button class="icon-btn" on:click={loadStations} title="Reload stations">↻</button>
    </div>

    <div class="row">
      <label class="field-label" for="wl-radio">Radio name</label>
      <input id="wl-radio" type="text" class="field-input short"
        value={activeProfile().wavelog_radioname}
        on:change={e => setProfileField('wavelog_radioname', e.target.value)} />
    </div>
  </section>
  ...
  <section>
    ...
    <div class="row">
      ...
      <select id="radio-type" class="field-input short" value={radioType}
    ...
    {#if radioType !== 'none'}
      <div class="row">
        ...
        <input id="radio-host" type="text" class="field-input short"
          value={radioType === 'flrig' ? activeProfile().flrig_host : activeProfile().hamlib_host}
          on:change={e => setProfileField(radioType === 'flrig' ? 'flrig_host' : 'hamlib_host', e.target.value)} />
        <label class="field-label" for="radio-port" style="margin-left:8px">Port</label>
        <input id="radio-port" type="text" class="field-input xshort"
          value={radioType === 'flrig' ? activeProfile().flrig_port : activeProfile().hamlib_port}
          on:change={e => setProfileField(radioType === 'flrig' ? 'flrig_port' : 'hamlib_port', e.target.value)} />
      </div>
      ...
      <div class="row checkrow">
        <label>
          <input type="checkbox"
            ...
            on:change={e => setProfileField('wavelog_pmode', e.target.checked)} />
          Set MODE on QSY
        </label>
        {#if radioType === 'hamlib'}
          <label>
            <input type="checkbox"
              ...
              on:change={e => setProfileField('ignore_pwr', e.target.checked)} />
            Ignore Power
          </label>
        {/if}
      </div>
  ...
  <div class="bottom-bar">
    ...
    {/if}
    {#if testMsg}
      <div class="alert" class:alert-success={testSuccess} class:alert-danger={testSuccess === false} class:alert-info={testSuccess === null} style="display:inline-block;padding:2px 8px">{testMsg}</div>
    {/if}
  </div>
</div>
{/if}
...
<!-- Profile Modal -->
{#if showProfileModal}
<div class="modal-overlay" on:click|self={() => showProfileModal = false}
  on:keydown={e => e.key === 'Escape' && (showProfileModal = false)} role="dialog" aria-modal="true">
  <div class="modal">
    <h4>Profiles</h4>
    ...
    <div class="new-profile-row">
      <input type="text" bind:value={newProfileName} placeholder="New profile name" class="rename-input" />
...
<div class="modal-overlay" on:click|self={() => showAdvancedModal = false}
  on:keydown={e => e.key === 'Escape' && (showAdvancedModal = false)} role="dialog" aria-modal="true">
  <div class="modal">
    ...
    <div class="row">
      ...
      <input id="adv-port" type="number" class="field-input xshort" bind:value={advUdpPort}
        min="1024" max="65535" disabled={!advUdpEnabled} />
    </div>
